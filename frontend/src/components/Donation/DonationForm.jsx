...
import { toast } from 'react-toastify'; // Assuming toastify is being used for notifications
import MiniLoader from '../ui/MiniLoader';
import { createRequest } from '../../secure/handleBackend';
...
const DonationForm = () => {
  ...
  const [isFailed, setIsFailed] = useState(false);  // New state for failure tracking
  ...
  const handleSubmit = async (e) => {
    ...
    setIsFailed(false); 
    try {
      const data = await createRequest(title, amount, reason, ethAddress);
      setIsSubmitting(false);
      if (data.success) {
        setIsSuccess(true);
        setIsFailed(false);  // Reset isFailed if the request is successful
      } else {
        setIsSuccess(false);
        setIsFailed(true);  // Set isFailed to true if request fails
      }
    } catch (error) {
      toast.error('There was a problem submitting your donation request.');
      setIsSubmitting(false);
      setIsSuccess(false);
      setIsFailed(true);  // Set isFailed to true if there is an error
    }
  };
  ...
  return (
    <div className="relative w-full max-w-md mx-auto bg-gray-800 text-white p-6 rounded-lg shadow-lg">
      <div className="text-center mb-6">
        <p className="text-gray-400">Fill out the form to create a new donation request</p>
      </div>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label htmlFor="ethAddress" className="block text-sm font-medium text-gray-300 mb-1">
            Ethereum Address
          </label>
          <input
            id="ethAddress"
            ...
            className="w-full bg-gray-700 text-white border-gray-600 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          {errors.ethAddress && <p className="mt-1 text-sm text-red-500">{errors.ethAddress}</p>}
        </div>
